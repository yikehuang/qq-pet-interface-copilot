...
但当前公开实现的 Android 登录只支持 QQ 密码登录并依赖外部现代 Android 签名
服务；二维码登录明确属于 PC 客户端路径。项目不会保存或收集 QQ 密码，也不会把
高风险第三方签名服务作为一键安装的默认依赖。因此现阶段先保留后端隔离与只读
白名单，不将 Acidify 或任何未验证签名服务直接打包进 Release。

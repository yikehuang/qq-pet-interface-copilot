...
## 纯电脑 Android 协议实验（2026-08-22）

`protocol-host/` 已完成回环 HTTP 边界、二维码登录流程、DPAPI 加密会话、好友列表、
OIDB 只读/写入双白名单与 Windows 构建测试。实网验证时，Android Phone 8.9.63 和
8.9.90 的本机 qsign 均可计算签名，但 QQ 登录服务器拒绝旧式 MiraiGo 二维码请求并
返回 `wtlogin.trans_emp sub cmd 0x31 error: 1`。因此该实验尚未建立在线会话，不能
作为 Release 的默认连接方式。下一步需要实现或接入当前 QQ 的新二维码登录流程，
随后才能验证 `0x95e1_0`；在此之前不得开放自动任务写入。

### 当前 Android NTQQ 实现评估

已核对 LagrangeDev 当前维护的 `acidify`（提交
`4e81a1bb9a9d1fda51bded3dac2a7f9163b99185`）。它具备 Android Phone AppID 16、
会话仓库、外部 Android Sign Provider 与自定义原始 SSO 封包能力，许可证为
GPL-3.0，技术和许可方向均比 MiraiGo 更适合本项目。

当前已加入 Acidify 1.6.3 Android Phone 9.2.80 协议桥、本机回环 HTTP 服务、
DPAPI 会话导入和恢复、好友列表与 OIDB 双白名单。启动器能够按锁文件自动安装
经过 SHA-256 校验的 Node.js 22.23.2 和 Acidify 依赖，并会拒绝包含 QQ 密码的
导入文件。二维码登录仍明确属于 PC 客户端路径，因此纯电脑 Android 后端采用
“一次性导入已授权会话”的方式，不收集或保存 QQ 密码。

高风险第三方签名服务仍不会作为一键安装依赖。当前代码默认关闭写操作，并且尚未
取得可公开复现的已授权 Android 会话完成实网 `0x95e1_0` 验证，因此不能把“本机
服务启动成功”描述成“QQ 宠物接口已经可用”。

进入可用状态前必须同时满足：无密码持久化、Android 会话可安全建立并续期、签名
版本与协议严格一致、真实 `0x95e1_0` 只读响应成功，以及主账号风险评估通过。
